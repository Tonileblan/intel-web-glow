...
export function QuickAuditForm() {
  ...
  return (
    <div className="mx-auto w-full max-w-4xl overflow-hidden rounded-2xl border border-slate-800 bg-slate-900/90 shadow-2xl backdrop-blur-xl">
      <div className="grid gap-0 md:grid-cols-12">
        {/* Left Side: Value & Guarantee */}
        <div className="flex flex-col justify-between border-b border-slate-800 bg-slate-950 p-6 text-slate-100 sm:p-8 md:col-span-5 md:border-b-0 md:border-r">
          <div>
            <div className="inline-flex items-center gap-1.5 rounded-full bg-red-500/15 border border-red-500/30 px-3 py-1 font-mono text-xs font-semibold text-red-400">
        ...
        <div className="p-6 sm:p-8 md:col-span-7">
          {submitted ? (
            <div className="flex min-h-[360px] flex-col items-center justify-center text-center">
              <div className="flex size-14 items-center justify-center rounded-full bg-emerald-950/60 border border-emerald-500/40 text-emerald-400">
              ...
              <button
                ...
                className="mt-6 inline-flex items-center rounded-lg border border-slate-700 bg-slate-800 px-4 py-2 text-xs font-semibold text-white hover:bg-slate-700"
              >
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
                  ...
                  {types.map((t) => (
                    <button
                      ...
                      className={`rounded-lg px-3 py-1.5 text-xs font-medium transition-all ${
                        selectedType === t.id
                          ? "bg-primary font-semibold text-white shadow-md ring-1 ring-red-400/50"
              ...
              <div className="grid gap-3 sm:grid-cols-2">
                <div>
                  <label htmlFor="audit-name" className="block text-xs font-medium text-slate-300">
                    Nombre o Empresa *
                  </label>
                  <input
                    id="audit-name"
                    ...
                    className="mt-1 h-10 w-full rounded-lg border border-slate-800 bg-slate-950 px-3 text-sm text-white placeholder:text-slate-500 outline-none transition-all focus:border-red-500 focus:ring-2 focus:ring-red-500/20"
                  />
                </div>

                <div>
                  <label htmlFor="audit-phone" className="block text-xs font-medium text-slate-300">
              ...
              <div className="grid gap-3 sm:grid-cols-2">
                <div>
                  <label htmlFor="audit-email" className="block text-xs font-medium text-slate-300">
                    Email
                  </label>
                  <input
                    id="audit-email"
              ...
              <div>
                <label htmlFor="audit-notes" className="block text-xs font-medium text-slate-300">
                ...
                <textarea
                  ...
                  className="mt-1 w-full rounded-lg border border-slate-800 bg-slate-950 p-2.5 text-sm text-white placeholder:text-slate-500 outline-none transition-all focus:border-red-500 focus:ring-2 focus:ring-red-500/20"
                />
              </div>
              ...
              <button
                type="submit"
                disabled={loading}
                className="inline-flex min-h-11 w-full items-center justify-center gap-2 rounded-lg bg-primary px-6 text-sm font-semibold text-white shadow-lg transition-all hover:bg-red-600 hover:shadow-red-600/30 disabled:opacity-50"
              >
                {loading ? (
